...
class Database:
    ...
    def guild_check(self, id_guild, nome):
        ...
        cursor.execute(f'SELECT EXISTS(SELECT * from Bot_Guild WHERE ID_Guild= "{id_guild}")')

        if (cursor.fetchall()[0][0] == 1):                                                  # Se já estiver cadastrado o ID
            cursor.execute(f'SELECT EXISTS(SELECT * from Guild_Aliases WHERE ID_Guild= "{id_guild}" AND AliasG = "{nome}")')

            if (cursor.fetchall()[0][0] == 0):                                              # SE não estiver cadastrado o Alias
                            cursor.execute(f'INSERT INTO Guild_Aliases VALUES ("{id_guild}", "{nome}")')

        else:
            cursor.execute(f'INSERT INTO Bot_Guild VALUES ("{id_guild}")')         #Registra o ID
            cursor.execute(f'INSERT INTO Guild_Aliases VALUES ("{id_guild}","{nome}")')    #Registra o Alias
        cursor.close()
        self.db.commit()
    ...
    def Comando_insert(self, bus):
        ...
        print (f'INSERT INTO Bot_Comando VALUES ("{bus[0]}", {str(bus[1])})')
        cursor.execute(f'SELECT NOT EXISTS(SELECT * FROM Bot_Comando WHERE nome = "{bus[0]}")')
        if (cursor.fetchall()[0][0] == 1):
            cursor.execute(f'INSERT INTO Bot_Comando VALUES ("{bus[0]}", {str(bus[1])})')
            cursor.execute(f'SELECT EXISTS(SELECT * FROM Bot_Comando WHERE nome = "{bus[0]}")')
            a = deepcopy(cursor.fetchall()[0][0])
            self.close(cursor)
            return a #1 se funcionou
        else:
            cursor.execute(f'SELECT EXISTS(SELECT * FROM Bot_Comando WHERE nome = "{bus[0]}" AND tempo = {bus[1]})')
            a = deepcopy(cursor.fetchall()[0][0])
            self.close(cursor)
            return a #1 se já existe

    async def Comando_register(self, comando, id_user, id_guild, repeticao):
        cursor = self.db.cursor()
        try:
            cursor.execute(f'SELECT EXISTS(SELECT * FROM Bot_Comando WHERE nome = "{comando}")')
            if cursor.fetchall()[0][0] == 1:
                cursor.execute(f'INSERT INTO Comando_Use VALUES ("{comando}", {str(repeticao)}, "{datetime.date(datetime.now())}", "{id_user}", "{id_guild}")')
            self.close(cursor)
        except:
